...
import { baseName, baseUrl, AppConfigApi } from '#/api/rp/appConfig';
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        return await AppConfigApi.getList({
...
async function handleDelete(row: any) {
  await AppConfigApi.delete(row.id);
    ...
    <BasicTable class="flex-1 overflow-hidden" :table-title="`${title}列表`">
      <template #toolbar-tools>
        <Space>
          <a-button
            type="primary"
            v-access:code="['rp:appConfig:add']"
            @click="handleAdd"
          >
            新增
          </a-button>
          <a-button
            ...
            danger
            type="primary"
            v-access:code="['rp:appConfig:remove']"
          ...
          >
            <a-button
              v-access:code="['rp:appConfig:export']"
              type="primary"
              color="red"
            >
      ...
      <template #action="{ row }">
        <Space>
          <a-button
            size="small"
            type="link"
            v-access:code="['rp:appConfig:edit']"
