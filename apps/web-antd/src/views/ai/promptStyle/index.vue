<script setup name="PromptStyle" lang="ts">
...
import { baseName, baseUrl, PromptStyleApi } from '#/api/ai/promptStyle';
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        return await PromptStyleApi.getList({
...
async function handleDelete(row: any) {
  await PromptStyleApi.delete(row.id);
...
async function handleMultiDelete() {
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      await PromptStyleApi.delete(ids);
      await tableApi.query();
    },
  });
}
    ...
    <BasicTable class="flex-1 overflow-hidden" :table-title="`${title}列表`">
      <template #toolbar-tools>
        <Space>
          <a-button
            type="primary"
            v-access:code="['ai:promptStyle:add']"
            @click="handleAdd"
          >
            新增
          </a-button>
          <a-button
            ...
            danger
            type="primary"
            v-access:code="['ai:promptStyle:remove']"
          ...
          >
            <a-button
              v-access:code="['ai:promptStyle:export']"
              type="primary"
              color="red"
            >
      ...
      <template #action="{ row }">
        <Space>
          <a-button
            size="small"
            type="link"
            v-access:code="['ai:promptStyle:edit']"
