...
import { ZC_TYPE } from '#/api/ai/promptStyle';
...
const changeKeywordsOptions = async (value: any, options: any) => {
  if (!value) {
    console.log(options);
    return;
  }
  ...
  <a-card class="config-panel overflow-y-scroll">
    ...
    >
      <div class="flex items-center">
        ...
        <h2
          class="bg-gradient-to-r from-emerald-400 to-blue-400 bg-clip-text text-2xl font-bold text-transparent"
        >
          种草配置
        </h2>
      </div>
        ...
        <a-col :span="24">
          <a-form-item label="🔗参考链接" name="noteId" :disabled="!visible">
            <div class="flex items-center justify-between">
              ...
              <button
                :disabled="!visible"
                class="group relative inline-flex items-center justify-center overflow-hidden rounded bg-gradient-to-r from-teal-400 to-blue-400 px-4 py-1 font-medium text-white hover:from-emerald-500 hover:to-blue-500"
...
.config-panel {
  ...
  background: linear-gradient(180deg, #fafbff 0%, #f6fdf7 90%);
}
...
// 生成按钮
.modern-generate-btn {
  @apply relative inline-flex w-[96%] items-center justify-center overflow-hidden rounded-lg bg-gradient-to-r from-teal-400 to-blue-400 px-6 py-3 font-medium text-white transition-all hover:scale-105 hover:from-teal-500 hover:to-blue-500 active:scale-95;
  &:hover {
    box-shadow: 0 4px 14px rgba(99, 241, 215, 0.25);
  }
}
...
.ant-switch-checked {
  ...
  &:hover:not(.ant-switch-disabled) {
    background-color: #34d399; /* 打开状态下鼠标悬停的背景色 */
  }
}
