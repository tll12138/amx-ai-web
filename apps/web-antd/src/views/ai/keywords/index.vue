<script setup name="Keywords" lang="ts">
...
import { Modal, Popconfirm, Select, SelectOptGroup, SelectOption, Space } from 'ant-design-vue';

import { useVbenVxeGrid, vxeCheckboxChecked } from '#/adapter/vxe-table';
import { baseName, baseUrl, KeywordsApi } from '#/api/ai/keywords';
  ...
  proxyConfig: {
    ajax: {
      query: async ({ page }, formValues) => {
        ...
        return await KeywordsApi.getList({
...
<template>
  <Page :auto-content-height="true">
    <BasicTable class="flex-1 overflow-hidden" :table-title="`${title}列表`">
      <template #toolbar-tools>
        <Space>
          ...
          <a-button
            ...
            danger
            type="primary"
            v-access:code="['ai:keywords:remove']"
          ...
          >
            <a-button
              v-access:code="['ai:keywords:export']"
              type="primary"
              color="red"
            >
      ...
      <template #form-productId="slotProps">
        <Select
          show-search
          allow-clear
          class="w-[80%]"
      ...
      <template #action="{ row }">
        <Space>
          <a-button
            size="small"
            type="link"
            v-access:code="['ai:keywords:edit']"
