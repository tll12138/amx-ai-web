...
import { baseName, baseUrl, GoodsApi } from '#/api/ai/goods';
...
<template>
  <Page :auto-content-height="true">
    <BasicTable class="flex-1 overflow-hidden" :table-title="`${title}列表`">
      <template #toolbar-tools>
        <Space>
          ...
          <a-button
            ...
            danger
            type="primary"
            v-access:code="['ai:goods:remove']"
            @click="handleMultiDelete"
          >
            删除
          </a-button>
          <a-button v-access:code="['ai:goods:add']" @click="showUploadDialog">
      ...
      <template #effect="{ row }">
        <Tag
          v-for="item in row?.effect?.split(' ')"
          :key="item"
          :bordered="false"
          color="processing"
        >
      ...
      <template #nickName="{ row }">
        <div v-if="row?.nickName" class="overflow-hidden overflow-ellipsis">
          <Tag
            v-for="item in row?.nickName"
            :key="item"
            :bordered="false"
            color="processing"
          >
