...
import { baseName, baseUrl, ModelApi } from '#/api/ai/model';
    ...
    <BasicTable class="flex-1 overflow-hidden" :table-title="`${title}列表`">
      <template #toolbar-tools>
        <Space>
          ...
          <a-button
            ...
            danger
            type="primary"
            v-access:code="['ai:model:remove']"
            @click="handleMultiDelete"
          >
            删除
          </a-button>
          <a-button v-access:code="['ai:model:add']" @click="showUploadDialog">
      ...
      <template #action="{ row }">
        <Space>
          <a-button
            :loading="loading"
            class="rounded-[10px] bg-green-500 px-3 py-1 text-white"
