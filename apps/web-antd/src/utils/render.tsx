import type { Component, Component as ComponentType } from 'vue';

import type { DictData } from '#/api/system/dict/dict-data-model';

import { h } from 'vue';

import { EllipsisText, JsonPreview } from '@vben/common-ui';
 ...
 * @param placement 位置
 */
export function renderEllipsisText(
  text: string,
  ...
  return (
    <EllipsisText
      placement={placement}
      tooltipOverlayStyle={{ whiteSpace: 'pre-wrap' }}
    >
      {text}{' '}
    </EllipsisText>
  );
...
/* 注意：这里只是拿到组件定义，不要加 JSX 括号 */
// 明确指定类型为 Component（Vue 组件）
const aiLogoMap: Record<string, Component> = {
...
export function renderModelLogo(text: string, model: string) {
  const IconComponent = aiLogoMap[text];

  if (IconComponent) {
    return (
      <div class="flex items-center justify-center font-semibold">
        {h(IconComponent, { class: 'text-[16px]' })}
...
export function renderAILogo(text: string) {
  const IconComponent = aiLogoMap[text];
  const element = renderDict(text, DictEnum.AI_PROVIDER);

  if (IconComponent) {
    return (
      <div class="flex items-center font-semibold">
        {h(IconComponent, { class: 'text-[16px]' })}
        <span class="ml-2">{element}</span>
      </div>
    );
  }
