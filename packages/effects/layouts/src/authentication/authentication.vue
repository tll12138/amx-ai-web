...
<template>
  ...
  >
    ...
    <div v-if="logo || appName" class="absolute left-0 top-0 z-10 flex flex-1">
      ...
      >
        <img v-if="logo" :alt="appName" :src="logo" class="mr-2" width="40" />
    ...
    <div v-if="authPanelCenter" class="flex-center relative w-full">
      <div
        class="addBg absolute inset-0 bg-cover bg-center bg-no-repeat"
      ></div>
      <div class="login-background absolute left-0 top-0 size-full"></div>
      <AuthenticationFormView
...
.addBg {
  background-image: url('http://10.10.4.32:9000/pai/login-bg.png'); /* 替换为你的图片路径 */
